...
(function() {
 ...
 default:    { emoji: '❔', color: '#95a5a6' }
    ...
    let activeUpcomingFilter = false; // new
    let activeTagFilter = null; // new: currently selected tag (null = all)
    let searchQuery = '';
    ...
    const upcomingFilterChip = document.getElementById('upcoming-filter'); // new
    const tagFilterContainer = document.getElementById('tag-filters'); // new
    ...
    function applyFilters() {
        ...
        // State filter (fixed to check location.state)
        ...
        if (activeUpcomingFilter) {
            ...
            result = result.filter(action => {
                const e = action.latestEntry;
                // Include if startDate is in the future or if there's a planned future entry
                const start = new Date(e.startDate);
                return start > now;
            });
        }
    ...
    function openDetailModal(actionId) {
                ...
                <div style="border-left: 2px solid #444; padding-left: 12px; margin-top: 6px;">
                    ${action.entries.map(entry => `
                        <div style="margin-bottom: 8px;">
                            <div style="font-weight: 600; color: #ccc;">${formatDate(entry.startDate)}</div>
                            <div style="font-size: 12px; color: #999;">${escapeHtml(entry.description)}</div>
                        </div>
                    `).join('')}
                </div>
            </div>`;
        ...
        detailModalContent.innerHTML = `
            ...
            <div class="detail-section"><div class="detail-label">Duration</div><div class="detail-value">${formatDate(latest.startDate)} — ${latest.endDate ? formatDate(latest.endDate) : 'Ongoing'}</div></div>
    ...
    function handleAddSubmit(e) {
        ...
        const tags = [];
        // Optional: add a tag input field in the form (not implemented here)
        // For now, we'll set tags empty or based on industry.
        ...
        buildGroupedActions();
        buildTagFilters(); // rebuild tag chips
        closeAddModal();
        applyFilters();
        updateTicker();
    }
